...
void main() {
	...
	float a = imageLoad(inTex,max(pt+ivec2(-1,-1),(0,0))).x;
	float b = imageLoad(inTex,max(pt+ivec2(-1, 0),(0,0))).x;
	float c = imageLoad(inTex,max(pt+ivec2(-1, 1),(0,0))).x;
	float d = imageLoad(inTex,max(pt+ivec2( 0,-1),(0,0))).x;
	float e = imageLoad(inTex,max(pt+ivec2( 0, 0),(0,0))).x;
	float f = imageLoad(inTex,max(pt+ivec2( 0, 1),(0,0))).x;
	float g = imageLoad(inTex,max(pt+ivec2( 1,-1),(0,0))).x;
	float h = imageLoad(inTex,max(pt+ivec2( 1, 0),(0,0))).x;
	float i = imageLoad(inTex,max(pt+ivec2( 1, 1),(0,0))).x;
    ...
    imageStore(destTex,pt,vec4(s, t, u, v));

}

//!DESC Anime4K-v3.1-Upscale(x2)-CNN-(L)-Conv-4x3x3x8
//////////////////////////////////////////////////////////////////
//!BIND HOOKED
...
void main() {
	ivec2 pt = ivec2(gl_GlobalInvocationID.xy);
	vec4 a0 = imageLoad(inTex,max(pt+ivec2(-1,-1),(0,0)));
	vec4 b0 = imageLoad(inTex,max(pt+ivec2(-1, 0),(0,0)));
	vec4 c0 = imageLoad(inTex,max(pt+ivec2(-1, 1),(0,0)));
	vec4 d0 = imageLoad(inTex,max(pt+ivec2( 0,-1),(0,0)));
	vec4 e0 = imageLoad(inTex,max(pt+ivec2( 0, 0),(0,0)));
	vec4 f0 = imageLoad(inTex,max(pt+ivec2( 0, 1),(0,0)));
	vec4 g0 = imageLoad(inTex,max(pt+ivec2( 1,-1),(0,0)));
	vec4 h0 = imageLoad(inTex,max(pt+ivec2( 1, 0),(0,0)));
	vec4 i0 = imageLoad(inTex,max(pt+ivec2( 1, 1),(0,0)));
    ...
    imageStore(destTex,pt,vec4(o0, p0, q0, r0));
}
...
//!DESC Anime4K-v3.1-Upscale(x2)-CNN-(L)-Conv-Reduce
//////////////////////////////////////////////////////////////////
//!BIND HOOKED
//!WHEN OUTPUT.w NATIVE.w / 1.200 > OUTPUT.h NATIVE.h / 1.200 > *
//!COMPONENTS 4
...
void main() {
	...
	vec4 b = imageLoad(inTex2,pt);
	vec4 c = imageLoad(inTex3,pt);
	vec4 d = imageLoad(inTex4,pt);
	vec4 e = imageLoad(inTex5,pt);
	vec4 f = imageLoad(inTex6,pt);
	vec4 g = imageLoad(inTex7,pt);
	vec4 h = imageLoad(inTex8,pt);
	vec4 i = imageLoad(inTex9,pt);
    vec4 na = -min(a, 0);
    vec4 nb = -min(b, 0);
    vec4 nc = -min(c, 0);
    vec4 nd = -min(d, 0);
    ...
    imageStore(destTex,pt,vec4(o, p, q, r));
...
{
	ivec2 pt = ivec2(gl_GlobalInvocationID.xy);
	vec4 result = imageLoad(inTex,pt);
	vec4 original = imageLoad(org,pt);

	imageStore(destTex,pt*2+ivec2(0,0),vec4(result.x+(9*original.x+3*get(-1, 0)+3*get( 0,-1)+get(-1,-1))/16.0,original.yz,0));
	imageStore(destTex,pt*2+ivec2(1,0),vec4(result.y+(9*original.x+3*get( 0,-1)+3*get( 1, 0)+get( 1,-1))/16.0,original.yz,0));
	imageStore(destTex,pt*2+ivec2(0,1),vec4(result.z+(9*original.x+3*get(-1, 0)+3*get( 0, 1)+get(-1, 1))/16.0,original.yz,0));
	imageStore(destTex,pt*2+ivec2(1,1),vec4(result.w+(9*original.x+3*get( 1, 0)+3*get( 0, 1)+get( 1, 1))/16.0,original.yz,0));
