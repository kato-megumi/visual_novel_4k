...
void main() {
	ivec2 pt = ivec2(gl_GlobalInvocationID.xy);
	if ((pt.x < p.x) || (pt.y < p.y) || (pt.x > p.z) || pt.y > p.w) return;
	vec4 a = imageLoad(inTex,max(pt+ivec2(-1,-1),(0,0)));
	vec4 b = imageLoad(inTex,max(pt+ivec2(-1, 0),(0,0)));
	vec4 c = imageLoad(inTex,max(pt+ivec2(-1, 1),(0,0)));
	vec4 d = imageLoad(inTex,max(pt+ivec2( 0,-1),(0,0)));
	vec4 e = imageLoad(inTex,max(pt+ivec2( 0, 0),(0,0)));
	vec4 f = imageLoad(inTex,max(pt+ivec2( 0, 1),(0,0)));
	vec4 g = imageLoad(inTex,max(pt+ivec2( 1,-1),(0,0)));
	vec4 h = imageLoad(inTex,max(pt+ivec2( 1, 0),(0,0)));
	vec4 i = imageLoad(inTex,max(pt+ivec2( 1, 1),(0,0)));
